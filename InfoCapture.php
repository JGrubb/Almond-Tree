 ...
 ***  Visit www.tectite.com for free <a href="http://www.tectite.com/">form script</a>.
 *
...
$TARGET_EMAIL = array(EMAIL_NAME."@almondtreemarketing\.com$");

            /* Help: http://www.tectite.com/fmdoc/def_alert.php */
define("DEF_ALERT","jgrubb@almondtreemarketing.com");
...
function LoadBuiltinLanguage()
{
    ...
    $aMessages[MSG_ABOUT_FORMMAIL] = '<a href="http://www.skylands.org/">Skylands Music and Arts Alliance</a>. '.
                                     ''.
                                     '';
...
$SPECIAL_FIELDS = array(
        ...
        "name",        // the real name of the person who filled in the form
    ...
    if (!isset($s_cols) || empty($s_cols) || !is_string($s_cols))
    {
        ...
        if (!isset($s_cols) || empty($s_cols) || !is_string($s_cols))
        {
            ...
            $s_cols = "";
                 ...
                 */
            $a_column_list = array("email","name");
                /*
                        ...
                        $s_to,$s_cc,$s_bcc,$a_raw_fields,$s_filter,$a_filter_list)
{
    ...
    if (!$b_used_template)
    {
        ...
        if (IsMailOptionSet("DupHeader"))
        {
            ...
            // (if not excluded)
            //
        if (!IsMailExcluded("name"))
        {
            array_unshift($a_fld_order,"name");
            $a_clean_fields["name"] = $SPECIAL_VALUES["name"];
        }
...
@   $log_fp = fopen($log_file,"a");
    ...
    $entry = $date.":".$SPECIAL_VALUES["email"].",".
            $SPECIAL_VALUES["name"].",".$SPECIAL_VALUES["subject"]."\n";
    fwrite($log_fp,$entry);
    fclose($log_fp);
}
